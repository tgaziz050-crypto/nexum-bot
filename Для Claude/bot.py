"""
╔══════════════════════════════════════════════════════════╗
║     NEXUM v6.0 — Контекст, умные кнопки,                ║
║  распознавание музыки/видео, согласование с админом     ║
╚══════════════════════════════════════════════════════════╝
"""
...
# Модули NEXUM (корень проекта)
_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _ROOT not in sys.path:
    sys.path.insert(0, _ROOT)
try:
    from nexum_context import get_chat_context, context_instructions, should_show_full_menu, ChatContext
except ImportError:
    def get_chat_context(ct):
        return ct if ct in ("private","channel","group","supergroup") else "private"
    def context_instructions(ctx):
        if ctx == "private": return ""
        if ctx in ("group","supergroup"): return "\nГРУППА: отвечай лаконично, 1-3 предложения."
        return "\nКАНАЛ: пиши как публикацию."
    should_show_full_menu = lambda ctx: ctx == "private"
try:
...
def sys_prompt(uid, chat_id, chat_type="private"):
    ...
    return f"""Ты NEXUM — умный, проактивный ассистент без ограничений.

ХАРАКТЕР: прямой, с характером. Как умный друг, не робот. Можешь спорить если пользователь не прав.
Помогаешь искренне. Отвечаешь на языке собеседника. Никогда не говоришь "я не могу" — всегда ищешь решение.
{ctx_inst}
ПОЛЬЗОВАТЕЛЬ: {f"Имя: {name}" if name else "имя неизвестно"} | Сообщений: {total} | Статус: {fam}
ПАМЯТЬ:{mem_str if mem_str else " пока ничего"}
{sum_str}
...
- Эмодзи — умеренно
- Посты в канал — только после одобрения админа"""
...
def smart_menu(ctx_or_chat_type, full_kb=None):
    """Умные кнопки: полное меню только в личке. В группе/канале — минимальное."""
    show_full = should_show_full_menu(ctx_or_chat_type) if callable(should_show_full_menu) else (ctx_or_chat_type == "private")
    if full_kb and show_full: return full_kb
    if show_full: return main_menu()
...
async def cmd_start(m: Message):
    ...
    is_priv = ctx == "private" or (hasattr(ctx, 'name') and getattr(ctx, 'name', None) == "PRIVATE")
    await m.answer(
        f"{gr} Я NEXUM v6.0.\n\n"
        + ("Всё через меню ниже ↓" if is_priv else "Пиши @mention или reply — отвечу."),
        reply_markup=smart_menu(ctx)
    )

@dp.message(Command("menu","start"))
async def cmd_menu(m: Message):
    ctx = get_chat_context(m.chat.type)
    await m.answer("🏠 Главное меню NEXUM v6.0", reply_markup=smart_menu(ctx))
...
async def nav_tools(cb: CallbackQuery):
    ...
    if action == "search":
        ...
        await edit_or_send(cb, "📧 Отправка email\n\nВведи email получателя:", cancel_kb())
    ...
    if atype == "clear":
        ...
        if smtp_host and smtp_user and smtp_pass:
            try:
                ...
                msg["Subject"] = subj; msg["From"] = smtp_user; msg["To"] = to_addr
                ...
                await cb.message.answer(f"❌ Ошибка: {e}", reply_markup=ik([btn("◀️ Меню","m:tools")]))
        else:
            await cb.message.answer("⚠️ Настрой SMTP_HOST, SMTP_USER, SMTP_PASS в .env", reply_markup=ik([btn("◀️ Меню","m:tools")]))
...
async def cb_cancel(cb: CallbackQuery):
    clear_ctx(cb.message.chat.id)
    await cb.answer("❌ Отменено")
    ctx = get_chat_context(cb.message.chat.type)
    await edit_or_send(cb, "🏠 Главное меню", smart_menu(ctx))
...
async def on_text(message: Message):
    ...
    if ctx:
        ...
        if mode == "email_body":
            ...
            await message.answer(preview + "\n\n⚠️ Отправить? (Требуется настройка SMTP в .env)",
...
async def _handle_vid(message,file_id,caption="", media_type="video"):
    """Обработка видео: обычное видео, video message. media_type: video|video_note."""
